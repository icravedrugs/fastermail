...
async function main(): Promise<void> {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/cleanup" && req.method === "GET") {
      ...
      try {
        const digestInfo = await store.getDigestDebugInfo();
        const orphaned = await store.getOrphanedEmails();

        // Also get labels from JMAP to check actual mailbox state
        const mailboxes = await jmap.getMailboxes();
        const fasterMailboxes = mailboxes.filter(m =>
          m.name?.startsWith("Fastermail") ||
        ...
        let html = `
          ...
          <body style="font-family: monospace; max-width: 900px; margin: 20px auto; padding: 0 20px;">
            <h1>Fastermail Debug Info</h1>

            <h2>Current Fastermail Mailboxes (JMAP)</h2>
            <table border="1" cellpadding="8" style="border-collapse: collapse;">
              <tr><th>Name</th><th>Total</th><th>Unread</th></tr>
              ${fasterMailboxes.map(m => `<tr><td>${m.name}</td><td>${m.totalEmails}</td><td>${m.unreadEmails}</td></tr>`).join('')}
            </table>

            <h2>Recent Digests (last 20)</h2>
            <table border="1" cellpadding="8" style="border-collapse: collapse;">
              <tr><th>ID</th><th>Status</th><th>Sent At</th><th>Cleaned At</th><th>Total Emails</th><th>By Classification</th></tr>
              ${digestInfo.map(d => `
                <tr style="background: ${d.status === 'sent' ? '#ffeeee' : d.status === 'pending' ? '#eeeeff' : '#eeffee'}">
                  <td>${d.id}</td>
                  <td><strong>${d.status}</strong></td>
                  <td>${d.sentAt || '-'}</td>
                  <td>${d.cleanedAt || '-'}</td>
                  <td>${d.totalEmails}</td>
                  <td>${JSON.stringify(d.byClassification)}</td>
                </tr>
              `).join('')}
            </table>

            <h2>Orphaned Emails (digests sent but not cleaned)</h2>
            ${orphaned.length === 0 ? '<p style="color: green;">No orphaned emails found!</p>' : `
              <p style="color: red;"><strong>${orphaned.reduce((sum, o) => sum + o.emails.length, 0)} total orphaned emails</strong></p>
              ${orphaned.map(o => `
                <h3>Digest ${o.digestId} (sent: ${o.sentAt})</h3>
                <p>${o.emails.length} emails:</p>
                <ul>
                  ${o.emails.slice(0, 20).map(e => `<li>[${e.classification}] ${e.subject} (from: ${e.fromEmail})</li>`).join('')}
                  ${o.emails.length > 20 ? `<li>... and ${o.emails.length - 20} more</li>` : ''}
                </ul>
              `).join('')}
            `}

            <h2>Analysis</h2>
            <p>Emails in "sent" but not "cleaned" digests remain in Fastermail/* folders.</p>
            <p>To fix: click archive on those digest emails, or manually clean up the labels.</p>

            <hr>
            <p style="color: #666;">Generated at: ${new Date().toISOString()}</p>
          </body>
          </html>
        `;
