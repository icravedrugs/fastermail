...
Generated by Fastermail${this.config.instanceId ? ` | Instance: ${this.config.instanceId}` : ""}`;
  ...
  async sendDigest(digest: Digest): Promise<void> {
    // Generate a unique send ID for this specific send operation
    const sendId = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

    console.log(`[SEND_START] digest_id=${digest.id}, send_id=${sendId}, instance=${this.config.instanceId}`);

    // Append send ID to both bodies for debugging duplicate sends
    const sendFooter = ` | Send: ${sendId}`;
    const htmlBody = digest.htmlBody.replace(
      /Generated by Fastermail/,
      `Generated by Fastermail${sendFooter}`
    );
    const textBody = digest.textBody.replace(
      /Generated by Fastermail/,
      `Generated by Fastermail${sendFooter}`
    );
    ...
    console.log(`[DRAFT_CREATED] draft_id=${draftId}, send_id=${sendId}`);

    const submissionId = await this.jmap.sendEmail(draftId);
    console.log(`[SEND_COMPLETE] digest_id=${digest.id}, send_id=${sendId}, submission_id=${submissionId}`);

    // Mark current digest as sent and create new pending digest for future emails
    await this.store.markDigestSent(digest.id, digest.totalEmails, textBody);
    console.log(`[MARKED_SENT] digest_id=${digest.id}, send_id=${sendId}`);

    await this.store.createPendingDigest();

    console.log(`Digest sent with ${digest.totalEmails} emails (send_id=${sendId})`);
  }
}
